...
# -----------------------
# helper: clean modelsummary export table
# removes unwanted bookkeeping columns like:
#   part = estimates / gof / manual
...
clean_modelsummary_table <- function(x) {
  ...
  # blank out the variable-name cell on statistic rows
  if (!is.null(stat_col)) {
...
# -----------------------
# 4) Combined / Retail / Institutional samples
# -----------------------
...
run_professor_table <- function(dtR_in, dtI_in, dtC_in, out_subdir) {
  ...
  # top border before summary statistics
