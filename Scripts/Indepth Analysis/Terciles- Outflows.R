# ===================== PROFESSOR TABLE: alpha_capm_12m (DAR OUTFLOW, T+1 DEP VAR, T REGRESSORS) =====================
# PURPOSE:
...
#   - Dependent variable = Darendeli outflow at t+1
...
# Outflow at t = magnitude of negative Darendeli flow
dt0[, outflow_dar_t := fifelse(
  is.na(flow_dar_t),
  NA_real_,
  pmax(-flow_dar_t, 0)
)]
...
dt0[, outflow_tp1 := shift(outflow_dar_t, 1L, type = "lead"), by = crsp_fundno]
...
keep_D <- c("outflow_tp1", "perf_t", "post_fund", "retail", "crsp_fundno", "crsp_portno", "ym", controls_m2_D)
...
run_professor_table <- function(dtR_in, dtI_in, dtC_in, dtD_in, out_subdir, fee_group = c("t1", "t2", "t3")) {
  ...
  OUT_DIR_RUN <- file.path(
    ...
    "Results",
    "professor_alpha_capm_12m_dar_outflow_tplus1_classFE",
  ...
  notes_txt <- paste0(
    ...
    "Darendeli outflow is defined as the magnitude of the negative part of that flow measure, i.e. max(-Darendeli flow, 0). ",
    "Performance variable: alpha_capm_12m at t. ",
  ...
  export_formatted_table(
    ...
    html_path = file.path(OUT_DIR_RUN, "professor_table_alpha_capm_12m_dar_outflow_tplus1_classFE.html"),
    tex_path  = file.path(OUT_DIR_RUN, "professor_table_alpha_capm_12m_dar_outflow_tplus1_classFE.tex"),
    xlsx_path = file.path(OUT_DIR_RUN, "professor_table_alpha_capm_12m_dar_outflow_tplus1_classFE.xlsx")
  )
  
  cat("\n===================== PROFESSOR TABLE BUILT =====================\n")
  cat("Output folder:\n", OUT_DIR_RUN, "\n\n")
  cat("Files created:\n")
  cat(" - professor_table_alpha_capm_12m_dar_outflow_tplus1_classFE.html\n")
  cat(" - professor_table_alpha_capm_12m_dar_outflow_tplus1_classFE.tex\n")
  cat(" - professor_table_alpha_capm_12m_dar_outflow_tplus1_classFE.xlsx\n\n")
...
# -----------------------
# 15) Build one combined tercile table for each model
# -----------------------
