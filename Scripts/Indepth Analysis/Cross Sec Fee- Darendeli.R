# ===================== PROFESSOR TABLE: alpha_capm_12m (DAR + T+1 FLOW, T REGRESSORS, HIGH- vs LOW-FEE) =====================
# PURPOSE:
...
#   - Fee split is unchanged: permanent high-fee / low-fee bucket based on pre-2024-07-30 average expense ratio
#   - Winsorization timing is unchanged: full dataset winsorized BEFORE splitting
...
# Darendeli flow at t+1 = dependent variable
...
# -----------------------
# 2B) High-fee vs low-fee split (TRUE CROSS-SECTION)
# Each class gets ONE permanent fee bucket for the whole sample
# -----------------------
...
# -----------------------
# 4) Samples for remaining three models
# -----------------------
...
run_professor_table <- function(dtR_in, dtI_in, dtC_in, out_subdir, fee_group = c("high", "low")) {
  ...
  fee_label <- if (fee_group == "high") "High-fee funds" else "Low-fee funds"
  
  notes_txt <- paste0(
    "Sample restricted to ", fee_label, ". ",
    "Fee bucket is permanent and based on each class's pre-2024-07-30 average expense ratio. ",
    "Dependent variable: Darendeli flow at t+1. ",
  ...
  table_title <- paste0("Table 5  Determinants analysis: ", fee_label)
  ...
  writeLines(
    ...
    con = file.path(OUT_DIR_RUN, "professor_table_alpha_capm_12m_dar_tplus1_classFE.html")
  )
  ...
  modelsummary(
    ...
    notes = notes_txt,
    title = table_title,
    output = file.path(OUT_DIR_RUN, "professor_table_alpha_capm_12m_dar_tplus1_classFE.tex")
  )
  ...
  fwrite(
    csv_out,
    file = file.path(OUT_DIR_RUN, "professor_table_alpha_capm_12m_dar_tplus1_classFE.csv")
  )
  ...
  saveWorkbook(
    wb,
    file.path(OUT_DIR_RUN, "professor_table_alpha_capm_12m_dar_tplus1_classFE.xlsx"),
    overwrite = TRUE
  )
  ...
  cat(" - professor_table_alpha_capm_12m_dar_tplus1_classFE.csv\n\n")
