# ===================== PROFESSOR TABLE: alpha_capm_12m (DAR FLOW t+1, t REGRESSORS, HIGH-vs-LOW FEE COMBINED TABLE) =====================
# PURPOSE:
...
#   - Output formatting upgraded; filenames reflect content
...
fit_fee_models <- function(dtR_in, dtI_in, dtC_in, fee_group = c("high", "low")) {
  ...
  dtR <- prepare_model_sample(
    ...
    core_keep = c("flow_tp1","perf_t","post_fund","crsp_fundno","crsp_portno","ym")
  )
  ...
  dtC <- prepare_model_sample(
    ...
    core_keep = c("flow_tp1","perf_t","post_fund","retail","crsp_fundno","crsp_portno","ym")
  )
...
export_combined_fee_table <- function(res_high, res_low, z_tests, out_subdir) {
  ...
  FILE_STEM <- "alpha_capm12m_dar_flow_tplus1_classFE_highlow_comparison"
  ...
  add_rows_main <- data.frame(
    ...
    "High fee - Combined" = c(
      "Yes","Yes","crsp_portno","Darendeli flow (t+1)",
  ...
  notes_txt <- paste0(
    ...
    "Dependent variable: Darendeli flow at t+1. ",
    "Darendeli flow is defined as (TNA_t - TNA_{t-1}(1+R_t)) / (TNA_{t-1}(1+R_t)), and the outcome uses its one-month lead. ",
