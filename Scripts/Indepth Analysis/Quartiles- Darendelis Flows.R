...
compute_z_from_models <- function(model_q4, model_q1, parts) {
  q4 <- extract_term_est_se(model_q4, parts = parts)
  q1 <- extract_term_est_se(model_q1, parts = parts)
  
  diff <- q4$beta - q1$beta
  zval <- diff / sqrt(q4$se^2 + q1$se^2)
...
dt0[, flow_tp1 := shift(flow_dar_t, 1L, type = "lead"), by = crsp_fundno]
...
add_sample_specific_fee_tercile <- function(DT) {
  ...
  fee_class[, fee_tercile := fifelse(
    ...
    fifelse(avg_exp_ratio_t <= qcuts[1], 1L,
            fifelse(avg_exp_ratio_t <= qcuts[2], 2L,
                    fifelse(avg_exp_ratio_t <= qcuts[3], 3L, 4L)))
  )]
...
# -----------------------
# 7B) Winsorize full dataset BEFORE splitting
# -----------------------
winsor_vars <- c(
  "flow_tp1",
  "perf_t",
...
dtD_win <- dt0_win[(retail == 1 & inst == 0) | (inst == 1 & retail == 0)]
dtD_win[, retail := as.integer(retail == 1)]

port_mix_win <- unique(
  dtD_win[, .(crsp_portno, crsp_fundno, retail)]
)[
  ,
  .(
    n_retail_classes = uniqueN(crsp_fundno[retail == 1]),
...
mixed_portnos_win <- port_mix_win[n_retail_classes > 0 & n_inst_classes > 0, crsp_portno]
dtD_win <- dtD_win[crsp_portno %in% mixed_portnos_win]
dtD_win <- add_post_from_adoption(dtD_win)
dtD_win <- add_sample_specific_fee_tercile(dtD_win)

run_professor_table <- function(dtR_in, dtI_in, dtC_in, dtD_in, out_subdir, fee_group = c("q1", "q2", "q3", "q4")) {
  ...
  OUT_DIR_RUN <- file.path(
    ...
    paste0(fee_group, "_fee_quartile"),
    out_subdir
  )
  ...
  core_keep_R <- c("flow_tp1","perf_t","post_fund","crsp_fundno","crsp_portno","ym")
  ...
  core_keep_I <- c("flow_tp1","perf_t","post_fund","crsp_fundno","crsp_portno","ym")
  ...
  core_keep_C <- c("flow_tp1","perf_t","post_fund","retail","crsp_fundno","crsp_portno","ym")
  ...
  keep_D <- c("flow_tp1","perf_t","post_fund","retail","crsp_fundno","crsp_portno","ym", controls_m2_D)
  ...
  fml_C <- as.formula(paste0(
    "flow_tp1 ~ perf_t * post_fund + retail + ",
  ...
  notes_txt <- paste0(
    "Fee subsample: ", toupper(fee_group), " quartile of pre-TSR average expense ratio. ",
    "Dependent variable: Darendeli flow at t+1. ",
  ...
  export_formatted_table(
    ...
    add_rows = add_rows,
    notes_txt = notes_txt,
    title_text = paste0("Table 5  Determinants analysis: ", toupper(fee_group), " fee quartile"),
...
build_tercile_summary_tables <- function(results_list, out_subdir) {
  ...
  model_sets <- list(
    retail = list(
      "Q1" = results_list$q1$m_retail,
      "Q2" = results_list$q2$m_retail,
      "Q3" = results_list$q3$m_retail,
      "Q4" = results_list$q4$m_retail
    ),
    inst = list(
      "Q1" = results_list$q1$m_inst,
      "Q2" = results_list$q2$m_inst,
      "Q3" = results_list$q3$m_inst,
      "Q4" = results_list$q4$m_inst
    ),
    combined = list(
      "Q1" = results_list$q1$m_combined,
      "Q2" = results_list$q2$m_combined,
      "Q3" = results_list$q3$m_combined,
      "Q4" = results_list$q4$m_combined
    ),
    did = list(
      "Q1" = results_list$q1$m_did,
      "Q2" = results_list$q2$m_did,
      "Q3" = results_list$q3$m_did,
      "Q4" = results_list$q4$m_did
    )
  )
  
  titles <- c(
    retail   = "quartile_summary_model1_retail",
    inst     = "quartile_summary_model1_institutional",
    combined = "quartile_summary_combined_classes",
    did      = "quartile_summary_model2_did"
  )
  
  pretty_titles <- c(
    retail   = "Table 5  Quartile summary: Model 1 - Retail",
    inst     = "Table 5  Quartile summary: Model 1 - Institutional",
    combined = "Table 5  Quartile summary: Combined classes",
    did      = "Table 5  Quartile summary: DiD - Model 2"
  )
  ...
  for (nm in names(model_sets)) {
    ...
    if (nm == "did") {
      zt <- compute_z_from_models(ms[["Q4"]], ms[["Q1"]], parts = c("perf_t", "post_fund", "retail"))
      diff_label <- "Coeff. difference on Perf (t) × Post × Retail [Q4 - Q1]"
      p_label    <- "P-value for coeff. difference [Q4 - Q1 pair]"
    } else {
      zt <- compute_z_from_models(ms[["Q4"]], ms[["Q1"]], parts = c("perf_t", "post_fund"))
      diff_label <- "Coeff. difference on Perf (t) × Post [Q4 - Q1]"
      p_label    <- "P-value for coeff. difference [Q4 - Q1 pair]"
    }
    ...
    notes_txt <- paste0(
      "Columns are fee quartiles based on pre-TSR average expense ratio. ",
      "Dependent variable: Darendeli flow at t+1. ",
      "Darendeli flow is defined as (TNA_t - TNA_{t-1}(1+R_t)) / (TNA_{t-1}(1+R_t)). ",
      "Performance variable: alpha_capm_12m at t. ",
      "Controls are measured at t. ",
      "All models use class fixed effects (crsp_fundno) plus year-month fixed effects. ",
      "Q1 vs Q4 coefficient comparison is reported using the p-value from the professor-requested test based on ",
      "z = (Beta_Q4 - Beta_Q1) / sqrt(SE_Q4^2 + SE_Q1^2). The p-value is shown once for the Q1-Q4 pair. ",
      "t statistics in parentheses. Standard errors are clustered by crsp_portno."
    )
