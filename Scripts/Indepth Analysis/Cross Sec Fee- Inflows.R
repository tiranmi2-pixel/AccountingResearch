# ===================== PROFESSOR TABLE: alpha_capm_12m (INFLOW t+1, t REGRESSORS, HIGH-vs-LOW FEE COMBINED TABLE) =====================
# PURPOSE:
...
fit_fee_models <- function(dtR_in, dtI_in, dtC_in, fee_group = c("high", "low")) {
  ...
  dtR <- prepare_model_sample(
    ...
    core_keep = c("inflow_tp1","perf_t","post_fund","crsp_fundno","crsp_portno","ym")
  )
  ...
  dtC <- prepare_model_sample(
    ...
    core_keep = c("inflow_tp1","perf_t","post_fund","retail","crsp_fundno","crsp_portno","ym")
  )
...
export_combined_fee_table <- function(res_high, res_low, z_tests, out_subdir) {
  ...
  FILE_STEM <- "alpha_capm12m_inflow_tplus1_classFE_highlow_comparison"
  ...
  add_rows_main <- data.frame(
    ...
    "High fee - Combined" = c(
      "Yes","Yes","crsp_portno","Inflow (t+1)",
  ...
  notes_txt <- paste0(
    ...
    "and the dependent variable is its one-month lead, inflow at t+1. ",
