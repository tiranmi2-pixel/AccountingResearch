...
# PURPOSE:
#  1) Add control variables to dt in memory (same logic)
...
# ===================== add_controls_monthly (same calculations; order rule adjusted) =====================
add_controls_monthly <- function(dt,
  ...
  # -----------------------
  # B) Expense + turnover from WRDS (same logic + update-join)
  # -----------------------
  ...
  if (!is.null(wrds)) {
    on.exit({
      if (close_wrds_on_exit) DBI::dbDisconnect(wrds)
    }, add = TRUE)
    ...
    ", fs_date_col, exp_col, turn_col, fs_table, fs_date_col, start_date, end_date))
    ...
    dt[, exp_ratio  := nafill(exp_ratio,  type="locf"), by=crsp_fundno]
    dt[, turn_ratio := nafill(turn_ratio, type="locf"), by=crsp_fundno]
  ...
  # -----------------------
  # C) Optional front/rear loads (same update-join logic)
  # -----------------------
  if (include_loads && !is.null(wrds)) {
    ...
    if (front_exists) {
      fl_cols <- dbGetQuery(wrds, "
        SELECT column_name
        FROM information_schema.columns
        WHERE table_schema='crspq' AND table_name='front_load'
      ")$column_name
      
      fl_date_col <- if ("caldt" %in% fl_cols) "caldt"
      else if ("date" %in% fl_cols) "date"
      else if ("qdate" %in% fl_cols) "qdate"
      else NA_character_
      
      fl_load_col <- c("front_load","max_front_load","load","max_load")[c("front_load","max_front_load","load","max_load") %in% fl_cols][1]
      stopifnot(!is.na(fl_date_col), !is.na(fl_load_col))
      
      front <- dbGetQuery(wrds, sprintf("
        SELECT crsp_fundno, %s AS caldt, %s AS front_load
        FROM crspq.front_load
        WHERE %s BETWEEN '%s' AND '%s'
      ", fl_date_col, fl_load_col, fl_date_col, start_date, end_date))
    ...
    if (rear_exists) {
      rl_cols <- dbGetQuery(wrds, "
        SELECT column_name
        FROM information_schema.columns
        WHERE table_schema='crspq' AND table_name='rear_load'
      ")$column_name
      
      rl_date_col <- if ("caldt" %in% rl_cols) "caldt"
      else if ("date" %in% rl_cols) "date"
      else if ("qdate" %in% rl_cols) "qdate"
      else NA_character_
      
      rl_load_col <- c("rear_load","max_rear_load","cdsc","load","max_load")[c("rear_load","max_rear_load","cdsc","load","max_load") %in% rl_cols][1]
      stopifnot(!is.na(rl_date_col), !is.na(rl_load_col))
      
      rear <- dbGetQuery(wrds, sprintf("
        SELECT crsp_fundno, %s AS caldt, %s AS rear_load
        FROM crspq.rear_load
        WHERE %s BETWEEN '%s' AND '%s'
      ", rl_date_col, rl_load_col, rl_date_col, start_date, end_date))
  ...
  # Preserve existing order; append ONLY newly created columns to the right.
  # -----------------------
